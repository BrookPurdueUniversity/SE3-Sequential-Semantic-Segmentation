...
# Overall functions should at least contain: 1-training, 2-saving model, 3-continue training.
#####
...
parser.add_argument('--num_val_images', type=int, default=620, help='The number of images to used for validations')
...
net_output = tf.placeholder(tf.float32,shape=[None,None,None,num_classes])   # initialize output tensor
network = model.build(inputs=net_input, num_classes=num_classes)   # recall customized model
loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=network, labels=net_output))  # define loss function
opt = tf.train.RMSPropOptimizer(learning_rate=0.0001, decay=0.995).minimize(loss, var_list=[var for var in tf.trainable_variables()])    # define optimizer for training
...
# Load a previous checkpoint if desired  (check if continue training!)
checkpoint_dir = args.checkpoint_dir
model_checkpoint_name = checkpoint_dir + '/latest_model.ckpt'
if args.continue_training:
    print('******************** Brook G. Cheng: Continue Training: Loaded latest model checkpoint ********************')
    saver.restore(sess, model_checkpoint_name)
else:
    print('********************  Brook G. Cheng: First-time Training: There are no latest model checkpoint ********************')
...
all_id_list = np.arange(0, len(input_names))  # return an array contains interger from start to end
...
for epoch in range(args.epoch_start_i, args.num_epochs):
    ...
    ## 3.1 generate the train and val
    train_input_names = []
    train_output_names = []
    val_input_names = []
    val_output_names = []
    ...
    # assign id for train and val dataset
    ...
    ## 3.2 prepare directory for saved model during training
    ...
    ## 3.3 Construct batch_szie for input_gt pair in training
    ...
    for i in range(num_iters):
        ...
        for j in range(args.batch_size):
            ...
            with tf.device('/cpu:0'):
                # Prep the data. Make sure:
                # input_image: normalized; float; >=4D_tensor;
                # label_image: one-hot; float; >=4Dtensor;
                input_image = np.float32(input_image) / 255.0   # image normalization and floatization
                output_image = np.float32(helpers.one_hot_it(label=output_image, label_values=label_values))  # label one-hot

                input_image_batch.append(np.expand_dims(input_image, axis=0))  # add one dimension as batch_size to form tensor
        ...
        ## 3.4 Do the training inside tensorflow container: feeding input_gt pair to container
        _,current=sess.run([opt,loss],feed_dict={net_input:input_image_batch,net_output:output_image_batch})
        
        # supervisd and displayed the training processes
    ...
    # if want to save model per each epoch      
    checkpoint_dir = args.checkpoint_dir
    ...
    if epoch % args.validation_step == 0:
        ...
        for ind in val_indices:
            ...
            input_image = np.expand_dims(np.float32(helpers.load_image(val_input_names[ind])),axis=0)/255.0   # validation-input: [1, H=256, W=256, C=3], float+normalized+tensor
            gt = helpers.load_image(val_output_names[ind]) 
            gt = helpers.reverse_one_hot(helpers.one_hot_it(gt, label_values))    # validation-gt: [H=256, W=256, C=1], revered one-hot, that is, each element of the HxW matrix is an interger index number of the semantic classes that pixel belongs to
            output_image = sess.run(network,feed_dict={net_input:input_image})  # validation-output: [1, H=256, W=256, C=num_class], float+normalized+tensor
            output_image = np.array(output_image[0,:,:,:])    # validation-output: [H=256, W=256, C=num_class], float+normalized
            output_image = helpers.reverse_one_hot(output_image)   # validation-output: [H=256, W=256, C=1], revered one-hot, it becomes an 2D indicator matrix of pixel-wise semantic class
            
            ## calculate valiation metrics
            ...
            ## visualized output image in validation
