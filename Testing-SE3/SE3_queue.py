...
M0_p = np.zeros([(2**0)+1, W, C], np.float32)                # [1, H=2, W=2048, C=3]
M0_c = np.zeros([(2**0)+1, W, 32], np.float32)               # [1, H=2, W=2048, C=32]
M0_c = tf.Variable(M0_c)

M1_p = np.zeros([(2**1)+1, W//(2**1), 32], np.float32)       # [1, H=3, W=1024, C=32]
M1_p = tf.Variable(M1_p)
M1_c = np.zeros([(2**1)+1, W//(2**1), 48], np.float32)       # [1, H=3, W=1024, C=48]
M1_c = tf.Variable(M1_c)

M2_p = np.zeros([(2**2)+1, W//(2**2), 48], np.float32)       # [1, H=5, W=512, C=48]
M2_p = tf.Variable(M2_p)
M2_c = np.zeros([(2**2)+1, W//(2**2), 96], np.float32)       # [1, H=5, W=512, C=72]
M2_c = tf.Variable(M2_c)

M3_p = np.zeros([(2**3)+1, W//(2**3), 96], np.float32)       # [1, H=9, W=256, C=72]
M3_p = tf.Variable(M3_p)
M3_c = np.zeros([(2**3)+1, W//(2**3), 128], np.float32)       # [1, H=9, W=256, C=96]
M3_c = tf.Variable(M3_c)

M4_p = np.zeros([(2**4)+1, W//(2**4), 128], np.float32)       # [1, H=17, W=128, C=96]
M4_p = tf.Variable(M4_p)
M4_c = np.zeros([(2**4)+1, W//(2**4), 192], np.float32)      # [1, H=17, W=128, C=128]
M4_c = tf.Variable(M4_c)

M5_p = np.zeros([(2**5)+1, W//(2**5), 192], np.float32)      # [1, H=33, W=64, C=128]
M5_p = tf.Variable(M5_p)
M5_c = np.zeros([(2**5)+1, W//(2**5), 256], np.float32)      # [1, H=33, W=64, C=192]
M5_c = tf.Variable(M5_c)

M6_p = np.zeros([(2**6)+1, W//(2**6), 256], np.float32)      # [1, H=65, W=32, C=192]
M6_p = tf.Variable(M6_p)
M6_c = np.zeros([(2**6)+1, W//(2**6), 320], np.float32)      # [1, H=65, W=32, C=256]
M6_c = tf.Variable(M6_c)

M7_p = np.zeros([(2**7)+1, W//(2**7), 320], np.float32)      # [1, H=129, W=16, C=256]
M7_p = tf.Variable(M7_p)
M7_c = np.zeros([(2**7)+1, W//(2**7), 384], np.float32)      # [1, H=129, W=16, C=320]
...
for file in files: 
    ...
    for i in range(0, H):
      ...
      if i >= 1:   # 2**1 lines
          #conv
          net = tf.nn.relu(conv(tf.expand_dims(M0_p, 0), w0, b0), name='ReLU')                              
          #update-conv
          M0_c = tf.concat([tf.slice(M0_c, [1,0,0], [-1,-1,-1]), tf.slice(tf.squeeze(net, 0), [0,0,0], [1,-1,-1])], axis=0)
          ...
          ### Encoding Block 1
          #pool
          net = slim.pool(tf.expand_dims(M0_c, 0), [s, s], stride=[s, s], pooling_type='MAX')
          #update-pool
          M1_p = tf.concat([tf.slice(M1_p, [1,0,0], [-1,-1,-1]), tf.squeeze(net, 0)], axis=0) 
          #prepare-conv
          net1 = tf.concat([tf.slice(M1_p, [0,0,0], [1,-1,-1]), tf.slice(M1_p, [2,0,0], [-1,-1,-1])], axis=0)     
          net1 = tf.expand_dims(net1, 0)
          if i >= 3:   # 2**2 lines
              #conv
              net = tf.nn.relu(conv(net1, w1, b1), name='ReLU')
              #update-conv             
              M1_c = tf.concat([tf.slice(M1_c, [1,0,0], [-1,-1,-1]), tf.slice(tf.squeeze(net, 0), [0,0,0], [1,-1,-1])], axis=0)     
              #prepare-pool 
              net = tf.concat([tf.slice(M1_c, [0,0,0], [1,-1,-1]), tf.slice(M1_c, [2,0,0], [-1,-1,-1])], axis=0)
              
              ### Encoding Block 2
              #pool
              net = slim.pool(tf.expand_dims(net, 0), [s, s], stride=[s, s], pooling_type='MAX')
              #update-pool
              M2_p = tf.concat([tf.slice(M2_p, [1,0,0], [-1,-1,-1]), tf.squeeze(net, 0)], axis=0)
              #prepare-conv
              net2 = tf.concat([tf.slice(M2_p, [0,0,0], [1,-1,-1]), tf.slice(M2_p, [4,0,0], [-1,-1,-1])], axis=0)     
              net2 = tf.expand_dims(net2, 0)
              if i >= 7:   # 2**3 lines
                  #conv
                  net = tf.nn.relu(conv(net2, w2, b2), name='ReLU')
                  #update-conv             
                  M2_c = tf.concat([tf.slice(M2_c, [1,0,0], [-1,-1,-1]), tf.slice(tf.squeeze(net, 0), [0,0,0], [1,-1,-1])], axis=0)     
                  #prepare-pool 
                  net = tf.concat([tf.slice(M2_c, [0,0,0], [1,-1,-1]), tf.slice(M2_c, [4,0,0], [-1,-1,-1])], axis=0)
                  
                  ### Encoding Block 3
                  #pool
                  net = slim.pool(tf.expand_dims(net, 0), [s, s], stride=[s, s], pooling_type='MAX')
                  #update-pool
                  M3_p = tf.concat([tf.slice(M3_p, [1,0,0], [-1,-1,-1]), tf.squeeze(net, 0)], axis=0)
                  #prepare-conv
                  net3 = tf.concat([tf.slice(M3_p, [0,0,0], [1,-1,-1]), tf.slice(M3_p, [8,0,0], [-1,-1,-1])], axis=0)     
                  net3 = tf.expand_dims(net3, 0)
                  if i >= 15:   # 2**4 lines
                      #conv
                      net = tf.nn.relu(conv(net3, w3, b3), name='ReLU')
                      #update-conv             
                      M3_c = tf.concat([tf.slice(M3_c, [1,0,0], [-1,-1,-1]), tf.slice(tf.squeeze(net, 0), [0,0,0], [1,-1,-1])], axis=0)     
                      #prepare-pool 
                      net = tf.concat([tf.slice(M3_c, [0,0,0], [1,-1,-1]), tf.slice(M3_c, [8,0,0], [-1,-1,-1])], axis=0)
                      
                      ### Encoding Block 4
                      #pool
                      net = slim.pool(tf.expand_dims(net, 0), [s, s], stride=[s, s], pooling_type='MAX')
                      #update-pool
                      M4_p = tf.concat([tf.slice(M4_p, [1,0,0], [-1,-1,-1]), tf.squeeze(net, 0)], axis=0)
                      #prepare-conv
                      net4 = tf.concat([tf.slice(M4_p, [0,0,0], [1,-1,-1]), tf.slice(M4_p, [16,0,0], [-1,-1,-1])], axis=0)     
                      net4 = tf.expand_dims(net4, 0)
                      if i >= 31:   # 2**5 lines
                          #conv
                          net = tf.nn.relu(conv(net4, w4, b4), name='ReLU')
                          #update-conv             
                          M4_c = tf.concat([tf.slice(M4_c, [1,0,0], [-1,-1,-1]), tf.slice(tf.squeeze(net, 0), [0,0,0], [1,-1,-1])], axis=0)     
                          #prepare-pool 
                          net = tf.concat([tf.slice(M4_c, [0,0,0], [1,-1,-1]), tf.slice(M4_c, [16,0,0], [-1,-1,-1])], axis=0)
                          
                          ### Encoding Block 5
                          #pool
                          net = slim.pool(tf.expand_dims(net, 0), [s, s], stride=[s, s], pooling_type='MAX')
                          #update-pool
                          M5_p = tf.concat([tf.slice(M5_p, [1,0,0], [-1,-1,-1]), tf.squeeze(net, 0)], axis=0)
                          #prepare-conv
                          net5 = tf.concat([tf.slice(M5_p, [0,0,0], [1,-1,-1]), tf.slice(M5_p, [32,0,0], [-1,-1,-1])], axis=0)     
                          net5 = tf.expand_dims(net5, 0)
                          if i >= 63:   # 2**6 lines
                              #conv
                              net = tf.nn.relu(conv(net5, w5, b5), name='ReLU')
                              #update-conv             
                              M5_c = tf.concat([tf.slice(M5_c, [1,0,0], [-1,-1,-1]), tf.slice(tf.squeeze(net, 0), [0,0,0], [1,-1,-1])], axis=0)     
                              #prepare-pool 
                              net = tf.concat([tf.slice(M5_c, [0,0,0], [1,-1,-1]), tf.slice(M5_c, [32,0,0], [-1,-1,-1])], axis=0)
                              
                              ### Encoding Block 6
                              #pool
                              net = slim.pool(tf.expand_dims(net, 0), [s, s], stride=[s, s], pooling_type='MAX')
                              #update-pool
                              M6_p = tf.concat([tf.slice(M6_p, [1,0,0], [-1,-1,-1]), tf.squeeze(net, 0)], axis=0)
                              #prepare-conv
                              net6 = tf.concat([tf.slice(M6_p, [0,0,0], [1,-1,-1]), tf.slice(M6_p, [64,0,0], [-1,-1,-1])], axis=0)     
                              net6 = tf.expand_dims(net6, 0)
                              if i >= 127:   # 2**7 lines
                                  #conv
                                  net = tf.nn.relu(conv(net6, w6, b6), name='ReLU')
                                  #update-conv             
                                  M6_c = tf.concat([tf.slice(M6_c, [1,0,0], [-1,-1,-1]), tf.slice(tf.squeeze(net, 0), [0,0,0], [1,-1,-1])], axis=0)     
                                  #prepare-pool 
                                  net = tf.concat([tf.slice(M6_c, [0,0,0], [1,-1,-1]), tf.slice(M6_c, [64,0,0], [-1,-1,-1])], axis=0)
                                    
                                  ### Encoding Block 7
                                  #pool
                                  net = slim.pool(tf.expand_dims(net, 0), [s, s], stride=[s, s], pooling_type='MAX')
                                  #update-pool
                                  M7_p = tf.concat([tf.slice(M7_p, [1,0,0], [-1,-1,-1]), tf.squeeze(net, 0)], axis=0)
                                  #prepare-conv
                                  net7 = tf.concat([tf.slice(M7_p, [0,0,0], [1,-1,-1]), tf.slice(M7_p, [128,0,0], [-1,-1,-1])], axis=0)     
                                  net7 = tf.expand_dims(net7, 0)
                                  if i >= 255:   # 2**8 lines
                                      #conv
                                      net = tf.nn.relu(conv(net7, w7, b7), name='ReLU')
                                      #update-conv             
                                      M7_c = tf.concat([tf.slice(M7_c, [1,0,0], [-1,-1,-1]), tf.slice(tf.squeeze(net, 0), [0,0,0], [1,-1,-1])], axis=0)     
                                      #prepare-pool 
                                      net = tf.concat([tf.slice(M7_c, [0,0,0], [1,-1,-1]), tf.slice(M7_c, [128,0,0], [-1,-1,-1])], axis=0)
                                      ...
                                      net = tf.concat([net, tf.expand_dims(M0_p, 0)], axis=-1)       # [1, H=2, W=2048, C=72+3=75]
